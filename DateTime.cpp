...
int DateTime::GetWeek() const {
    ...
    auto days_since_monday = (t.tm_wday + 6) % 7;
    auto yday =  t.tm_yday;
    auto yday_idx = yday % 7;
    auto first_week_offset = 7 - yday_idx;
    auto first_week_first_day = (days_since_monday + first_week_offset) % 7;
    yday += first_week_first_day;
    auto weeknum = yday / 7;
    if (first_week_first_day < 4) {
        ++weeknum;
    }
    return weeknum;
}

std::string DateTime::to_dateiso8601() const {
    struct tm t{};
    if (gmtime_r(&timeInSecondsSinceEpoch, &t) != &t) {
        throw std::exception();
    }
    std::string str{};
    AppendToString<10>(str, DateToIso8601(t.tm_year + 1900, t.tm_mon + 1, t.tm_mday));
    return str;
}
